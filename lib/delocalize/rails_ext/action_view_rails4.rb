...
ActionView::Helpers::Tags::TextField.class_eval do
  ...
  def render_with_localization
    if object && (@options[:value].blank? || !@options[:value].is_a?(String)) && object.respond_to?(:has_attribute?) && object.has_attribute?(@method_name) && object.respond_to?(:column_for_attribute) && column = object.column_for_attribute(@method_name)
